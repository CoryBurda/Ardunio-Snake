...
//    }
    ...
    Point<T> GetMidPoint(const Point<T>& p) const{
      Point<T> pret = (p - *this) / 2 + *this;
      return pret;
    }
    Point<int> GetMidPointInt(const Point<int>& p) const{
      Point<float> pretf = (p - *this) / 2 + *this;
      Point<int> pret(round(pretf.GetX()),round(pretf.GetY())); 
      return pret;
    }
    T GetDist(const Point<T>& p) const{
      return hypot(GetDistX(p), GetDistY(p));
    }
    T GetDist() const{
      return hypot(m_nX, m_nY);
      //return sqrt(m_nX * m_nX + m_nY * m_nY);
    }
    Point<T> GetaMidPoint(const Point<T>& p, const T& t) const{
